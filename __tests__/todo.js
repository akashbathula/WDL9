...
describe("Testing Todo ", () => {
  ...
  test("Updating the field that are completed in the given todo list : ", async () => {
    const res = await agent.get("/");
    const csrfToken = extractCsrfToken(res);
 ...
 //Test
  ...
  test("Noting the todo as complete", async () => {
  ...
  test("Marks todo as incomplete with the given ID", async () => {
    let res = await agent.get("/");
    let csrfToken = extractCsrfToken(res);
    await agent.post("/todos").send({
  ...
  //Test
  test("Deleting the todo using ID", async () => {
    let res = await agent.get("/");
    let csrfToken = extractCsrfToken(res);
    await agent.post("/todos").send({
